                ...
                for necesario in necesita:
                    if count <= tamano_R:
                        ...
                        if necesario == obtenido:
                            count +=1
                        elif rooms.nombre =="Plaza Rectorado" and lado == "center":
                            if "título Universitario" in tengo and "Mensaje: Si estas gradudado puedes pisar el Samán" in tengo: 
                                count +=1

                        if count == tamano_R:
                            salir = True
                            break
    elif necesita == "\nintroducir contrase\u00f1a de la computadora\n":
        for obtenido in tengo:
            ...
            if desicion == "0":
                ...
                if pasar(usuario, rooms, lado):
                    with open('status.json', 'r') as file:
                        data = json.load(file)
                    for nom in data[usuario]:
                        pasar1 = nom["Pasillo Laboratorios _center"]
                        if pasar1 == "1":
